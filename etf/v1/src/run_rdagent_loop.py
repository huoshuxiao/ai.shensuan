# -*- coding: utf-8 -*-
"""拉起一整轮 RD-Agent(Q) factor 循环（ETF 线），复用 official_rdagent 的环境/sg 逻辑
...
def main(loops=1):
    ...
    # 与主线同一套体检：任一项缺失就打印原因并退出，不拉起注定失败的循环
    for name, ok, detail in o.rdagent_preflight(out):
        print(f"  {'✅' if ok else '❌'} {name}: {detail}")
        if not ok:
            print("  ℹ️ 前置依赖缺失，本次未拉起循环")
            return 1
